...
//	- 메소드는 다른 메소드를 호출할 수 잇다.
//	- 해당 메소드를 호출해서 내용이 끝나면 메소드를 호출한 곳으로 되돌아 간다.
// 1) 메소드 구성 : [접근제한자] [메소드종류] 반환형 메소드 이름[(인자=매개변수)]{실행내용}
//   가) 접근제한자 : class 접근 제한자와 같음
//		 - public, protected, 생략, private
//	 나) 메소드 종류 : 일반메소드는 이 부분을 생략한다.
//				       특정 메소드일 때 특정메소드에 해당하는 예약어를 사용(static)
//   다) 반환형 : 해당 메소드를 호출한 곳으로 되돌아 갈 때 결과의 자료형을 말한다.
//				  만약에 결과를 가지고 가지 않으면 void라는 예약어를 사용한다.
//		예) public static void main(String[] args){}
//			=> 메소드 이름은 main이고 해당 메소드를 호출한 JVM에게 아무것도 가져가지 않는다.
//	 라) 메소드이름 : 첫글자 소문자 나머지 소문자, 두 단어 이상일 때 단어 첫 글자 대문자
// 	 마) 인자 : 메소드가 실행될 때 필요한 정보를 외부에서 받아서 사용 한다.
...
// static과 instance : 멤버메소드와 멤버 필드에 공통으로 해당 되는 사항
...
//		생성자가 없는 클래스는 기본 생성자가 생략되어 있는 것 이다.
